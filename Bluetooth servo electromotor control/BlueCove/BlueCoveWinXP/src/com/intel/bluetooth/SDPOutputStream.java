...
public class SDPOutputStream extends FilterOutputStream
{
	...
	private int getLength(DataElement e)
	{
		...
		case DataElement.DATALT:
		{
			...
			for(Enumeration enum = (Enumeration)e.getValue(); enum.hasMoreElements(); )
				result += getLength((DataElement)enum.nextElement());
	...
	public void writeElement(DataElement e) throws IOException
	{
		...
		case DataElement.U_INT_1:
			write(8|0);
			writeLong(e.getLong(), 1);
			break;
		case DataElement.U_INT_2:
			write(8|1);
			writeLong(e.getLong(), 2);
			break;
		case DataElement.U_INT_4:
			write(8|2);
			writeLong(e.getLong(), 4);
			break;
		case DataElement.U_INT_8:
			write(8|3);
			writeBytes((byte[])e.getValue());
			break;
		case DataElement.U_INT_16:
			write(8|4);
			writeBytes((byte[])e.getValue());
		...
		case DataElement.BOOL:
			write(40|0);
			writeLong(e.getBoolean() ? 1 : 0, 1);
			break;
			
		case DataElement.DATSEQ:
			write(48|7);
			writeLong(getLength(e)-5, 4);
			
			for(Enumeration enum = (Enumeration)e.getValue(); enum.hasMoreElements(); )
				writeElement((DataElement)enum.nextElement());

...
public class ServiceRecordImpl implements ServiceRecord
{
    ...
    */
    ...
    public int[] getAttributeIDs()
    {
        ...
        for(Enumeration enum = attributes.keys(); enum.hasMoreElements(); ) 
            attrIDs[i++] = ((Integer)enum.nextElement()).intValue();
    ...
    */
    ...
    public boolean populateRecord(int[] attrIDs) throws IOException
    {
        ...
        */
        ...
        byte[] blob = ((LocalDeviceImpl)LocalDevice.getLocalDevice()).getBluetoothPeer().getServiceAttributes(sortIDs, Long.parseLong(device.getBluetoothAddress(), 16), handle);
		
        if (blob.length > 0) 
            try {
                DataElement element = (new SDPInputStream(new ByteArrayInputStream(blob))).readElement();

                for(Enumeration enum = (Enumeration)element.getValue(); enum.hasMoreElements(); )
                    attributes.put(new Integer((int)((DataElement)enum.nextElement()).getLong()), enum.nextElement());
    ...
    */
    ...
    public String getConnectionURL(int requiredSecurity, boolean mustBeMaster)
    {
        ...
        */
        ...
        DataElement e1 = getAttributeValue(ProtocolDescriptorList);
		
        if (e1.getDataType() == DataElement.DATSEQ)
            for(Enumeration enum1 = (Enumeration)e1.getValue(); enum1.hasMoreElements(); ) {
                DataElement e2 = (DataElement)enum1.nextElement();
				
                if (e2.getDataType() == DataElement.DATSEQ) {
                    Enumeration enum2 = (Enumeration)e2.getValue();
					
                    if (enum2.hasMoreElements()) {
                        DataElement e3 = (DataElement)enum2.nextElement();
						
                        if (enum2.hasMoreElements() && e3.getDataType() == DataElement.UUID && e3.getValue().equals(UUID.RFCOMM_PROTOCOL_UUID)) {
                            DataElement e4 = (DataElement)enum2.nextElement();
							
                            switch(e4.getDataType()) {
    ...
    /*
      ...
      RuntimeExceptin - if the ServiceRecord receiving the message was obtained from a remote device
    */
    /*
    ...
    */
    ...
    public String toString()
    {
        ...
        for(Enumeration enum = attributes.keys(); enum.hasMoreElements(); ) {
            ...
            DataElement e = (DataElement)attributes.get(i);
